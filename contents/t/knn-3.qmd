...
LOWESS (Locally Weighted Scatterplot Smoothing) is a local regression method that combines local linear fitting with distance-based weights.

For a target point $x_0$, LOWESS first selects a neighborhood around $x_0$. The neighborhood size is determined by a span parameter, usually denoted by $\alpha$. If there are $n$ observations, the closest

$$
m = \lceil \alpha n \rceil
$$

points are used in the local fit.
...
be the distance from $x_0$ to the furthest point in the neighborhood. LOWESS assigns weights using the tricube kernel
...
`lowess` from `statsmodels.nonparametric.smoothers_lowess` can be used. Its API is very similar to `KernelReg`. 
...
```
`return_sorted` is for the format of the output. 

- If it is `True`, the output will be a 2 column `numpy.array`, that the first column is the sorted `X`, while the second column is the corresponding fitted `y`. 
- If it is `False`, the output will be a 1 column `numpy.array` with fitted `y` according to the original order of `X`.
- We could assign `xvals` for prediction. If `xvals` is provided, then `return_sorted` is ignored and the returned array is always one dimensional, containing the `y` values fitted at the `x` values provided by `xvals`.


::: {.callout-note}
## LOWESS is for visualizing smooth curves
The original idea of LOWESS is to smooth curves and for visualization. Therefore the default output format for `lowess` is `return_sorted=True`. In this case, the output `X` is sorted from low to high, that it is easy to directly plot the curve.

You may compare to `KernelReg` and other methods we mentioned earlier. In order to plot the resulting curve, we set up a grid of indepedent variables `X_grid`, and plot according to it. In the case of `lowess`, we don't need to introduce new variables, but directly use the sorted `X` from output of `lowess`.
:::


::: {.callout-note}

## LOWESS versus Kernel Regression
...
As a result, LOWESS often exhibits better boundary behavior and adapts naturally to regions with varying data density.
:::

LOWESS can optionally perform additional robustification iterations.

After an initial fit, observations with large residuals receive smaller weights. The final fit uses both distance-based weights and robustness weights. As a result, outliers have less influence on the fitted curve.

In `statsmodels`, LOWESS is implemented by `statsmodels.nonparametric.smoothers_lowess.lowess`. The argument `it` specifies the number of robustification iterations.
```{python}
#| eval: false
lowess(y, X, frac=0.25, it=0)  # ordinary LOWESS
lowess(y, X, frac=0.25, it=3)  # robust LOWESS
```

The parameter `it` controls robustification iterations. Most applications use either `it=0` for ordinary LOWESS or `it=3` for robust LOWESS. 
...
<!-- LOWESS stands for locally weighted scatterplot smoothing. It is another local regression smoother. In `statsmodels`, LOWESS is implemented by `statsmodels.nonparametric.smoothers_lowess.lowess`. The method uses local linear estimates; the argument `frac` controls the fraction of observations used in each local fit. -->
...
| 0.2–0.4 | Common choice for many datasets |
| 0.5–0.8 | Strong smoothing |
| >0.8 | Very smooth, may miss local structure |

The span parameter frac may be selected by cross-validation when prediction accuracy is the primary goal. However, LOWESS is often used as an exploratory visualization tool, and in practice the span is frequently chosen by comparing several values and inspecting the resulting curves.

When LOWESS is used for prediction, cross-validation plays the same role as bandwidth selection in kernel regression.

There is no universally optimal value. In practice, several values are often tried and compared visually.
