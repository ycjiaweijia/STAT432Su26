...
From a statistical learning perspective, these goals may be viewed as forming an approximate information hierarchy:

$$
\text{Probability Quality}
\Longrightarrow
\text{Potential for Good Ranking}
\Longrightarrow
\text{Potential for Good Decisions}.
$$

In general, richer probability information can support ranking and decision making across many thresholds or cost functions, although the relationships are not mathematically equivalent.
...
You may also use the `FixedThresholdClassifier` from `sklearn.model_selection` to wrap it.

```{python}
from sklearn.model_selection import FixedThresholdClassifier

threshold = 0.3
model_with_threshold = FixedThresholdClassifier(logit, threshold=threshold)
y_pred_wrapper = model_with_threshold.predict(X_test)
```
You can see that the results are the same.

```{python}
(y_pred==y_pred_wrapper).mean()
```

Note that `FixedThresholdClassifier` can be used to wrap an unfitted model so you may directly fit a model with different threshold.


::: {.callout-note}
For `FixedThresholdClassifier` that you already set the threshold any way, all three methods (manual code, wrap a fitted model, wrap a model and then fit) will give the same answer. 
