...
::: {.callout-tip}
In Python power operation is `**`. Therefore `3**2` is `9`.
:::
...
Instead of explicitly copying data, NumPy automatically expands arrays along dimensions of size 1 when the shapes are compatible. Two dimensions are compatible if they are equal or if one of them is 1.
...
Broadcasting can also occur between higher-dimensional arrays. For example,
...
Here `a` has shape `(1,3)` and `b` has shape `(2,1)`. These shapes are compatible because, along each axis, either the dimensions are equal or one of them is `1`.

Broadcasting expands dimensions of size `1` to match the corresponding dimension of the other array. In this example, the single row of `a` behaves as if it were repeated twice, while the single column of `b` behaves as if it were repeated three times, resulting in arrays of shape `(2,3)`.
