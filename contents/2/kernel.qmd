...
We first compute the raw weights according to $x_0=3$. Raw means that the weight is before normalization. The estimation $\hat f(x)$ is then computed.
...
We use the raw weights to generate the size of each point in order to visualize the weights of each point.
``` {python}


point_sizes = 200 * raw_weights / raw_weights.max()
```
With these info we could know plot the curve and the data with the assigned point sizes.
<!-- fmt: off -->
```{python}
