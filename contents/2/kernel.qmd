...
```
We then use the weights to determine the size of each plotted point, so that larger circles correspond to larger kernel weights. The factor 200 is chosen only for visualization.

<!-- We then use the weights to determine the size of each plotted point so that larger points correspond to larger kernel weights. 200 is chosen for better visualization. -->
``` {python}
