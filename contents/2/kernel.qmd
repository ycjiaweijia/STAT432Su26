...
For a target point (x_0), LOWESS first selects a neighborhood around (x_0). The neighborhood size is controlled by a span parameter, usually denoted by (\alpha). If there are (n) observations, the closest
...
The argument `return_sorted` controls the format of the output.
...
This differs slightly from the workflow used for `KernelReg` and other prediction-style methods. For those methods, we often create a grid such as `X_grid` and then predict on that grid. With LOWESS, we can often plot the sorted values returned by the function directly.
...
Most applications use either it=0 for ordinary LOWESS or it=3 for robust LOWESS.
