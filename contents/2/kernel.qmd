...
$$
\begin{split}
\Exp[\hat f(x)]&=\int K(u)\qty[f(x)-huf'(x)+\frac12h^2u^2f''(x)+O(h^3)]\dl3u\\
&=\int K(u)f(x)\dl3u-\int K(u)huf'(x)\dl3u+\int\frac12K(u) h^2u^2f''(x)\dl3u+\int K(u)O(h^3)\dl3u\\
&=f(x)\int K(u)\dl3u-f'(x)h\int uK(u)\dl3u+O(h^2).
\end{split}
$$

Because a symmetric kernel satisfies
