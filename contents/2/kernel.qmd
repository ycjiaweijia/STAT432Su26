...
We first compute the kernel weights for the target point $x_0=3$. Here, raw means that the weights have not yet been normalized. The estimate $\hat f(x_0)$ is then computed.
...
When prediction accuracy is important, cross-validation is often preferred because it provides a more data-driven approach to bandwidth selection.
...
The following code uses `GridSearchCV` to select the bandwidth. Since `sklearn` doesn't provide the API for Nadaraya-Watson estimator, in order to keep the usual `sklearn` workflow, we write a custom estimator. 
...
Then use the reguler grid search process.


### Bandwidth Selection with `statsmodels`

Rule-of-thumb methods are fast and easy to use, but they rely on assumptions that may not hold in practice. For kernel regression, `statsmodels` provides a direct implementation through `KernelReg`.

`KernelReg` can fit both local constant regression and local linear regression. The local constant version is the Nadaraya-Watson estimator. In `statsmodels`, this is specified by setting `reg_type="lc"`. The option `bw="cv_ls"` selects the bandwidth by least-squares cross-validation. The default regression type is local linear, `reg_type="ll"`, so we set `reg_type="lc"` explicitly when we want the Nadaraya-Watson estimator. :contentReference[oaicite:1]{index=1}
...
plt.axvline(silverman_h, color="red", linestyle="--", label='silverman_h')
