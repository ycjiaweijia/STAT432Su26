...
::: {.callout-note}
## Learning Rate and the $\frac12$ Factor
...
This does not fundamentally change the algorithm. The factor $\frac12$ can simply be absorbed into the learning rate by replacing $\eta$ with $\eta/2$. Therefore the two formulas differ only by a rescaling of the step size.

We mainly use the classical form with $\frac12$ since it naturally appears in the standard derivation from exponential loss minimization and margin theory.
...
- Bagging and Random Forests mainly reduce variance by averaging many trees independently built.
...
<!-- Common values are 0.01, 0.05, 0.1, 1.0, etc.. -->
...
The new tree $f_m(x)$ is chosen to reduce the loss function. Ideally, we would minimize
$$
l(y_i,F_m(x_i)+f_m(x_i))
$$
for each $f_m(x_i)$. Gradient boosting approximates this optimization using the idea of gradient descent: the predictions $f_m(x_i)$ should approximate the negative gradient, which gives the direction of steepest descent in the loss. We therefore define the pseudo-residual for the observation $x_i$ as
...
\right|_{F=F_m}.
$$
The pseudo-residuals $r_i$ are then used as the target values when fitting the new regression tree $f_m$.
...
Substituting this into the quadratic approximation @eq-xg_loss and ignoring constant terms gives the approximate objective
...
- $\gamma T$ penalizes the number of leaves. The parameter $\gamma$, called the minimum split loss, specifies the minimum reduction in the objective required for an additional split.
- $\frac{1}{2}\lambda\sum_{j=1}^{T}s_j^2$ penalizes large leaf weights. The parameter $\lambda$ controls the strength of this $L_2$ regularization.
...
Including both $L_1$ and $L_2$ regularization, the full penalty function becomes
...
The $L_1$ penalty can shrink some leaf weights exactly to zero, making the contribution of those leaves vanish. It may improve robustness and reduce overfitting, particularly when the data contain substantial noise.

This penalty is analogous to the $L_1$ penalty used in lasso regression. However, unlike lasso regression, it regularizes leaf values rather than individual feature coefficients, so it does not directly perform feature selection.

We initially omit $L_1$ regularization because its nondifferentiability at zero makes the derivation of the optimal leaf weights and split gains more complicated. Nevertheless, $\alpha$ is a hyperparameter worth considering when tuning an `XGBoost` model.
:::
...
| `gamma` | Minimum split loss for the tree. |
...
## `subsample`
`subsample` controls the fraction of training observations randomly sampled. For example `subsample=0.8` means that each new tree is trained using only 80% of the observations. Sampling is performed without replacement, and a new sample is drawn for each boosting round.
...
## `colsample_bytree`
`colsample_bytree` controls the fraction of predictors randomly sampled for each tree. For example `colsample_bytree=0.7` means that each tree only considers 70% of the predictors. Sampling is performed without replacement, and a new set of predictors is drawn for each tree.

This idea is similar to feature subsampling in Random Forests. It reduces correlation between trees and can improve robustness and generalization performance. However, Random Forests commonly sample predictors separately at each split, whereas `colsample_bytree` selects one set of predictors for the entire tree (and therefore its name is `bytree`). 
...
Similar to `bagging`, for a nested structure such as `GridSearchCV` with `XGBoost`, we normally use multiple cores only at the outermost layer, that is, `GridSearchCV`. However, some studies suggest enabling multicore processing in `XGBoost` while using a single core for `GridSearchCV` when the dataset is very large.
:::
...
### `RandomizedSearchCV`
We already show the code of `RandomizedSearchCV` before. Here we give more explanations. 

`RandomizedSearchCV` is to search random combinations of hyperparameters. Therefore it is used to do a rough search in a very large parameter space. Other than the `param_grid` like `GridSearchCV`, `RandomizedSearchCV` needs `n_iter` to indicate how many combinations one wants to test.
...
param_distributions = {
    ...
    "gamma": [0, 0.01, 0.05, 0.1, 0.3, 0.5, 1, 2, 3, 5],
...
A common choice for exploring positive values of `gamma` is a log-uniform distribution. However, `gamma=0` is also a plausible and often competitive choice, and a log-uniform distribution cannot include `0`. We therefore use a discrete candidate set containing zero together with positive values spanning several orders of magnitude. If there is a strong reason to exclude `gamma=0`, `loguniform` can be used directly.

Similarly `reg_alpha` has similar issues that `0` (hence no $L_1$ regularization) is usually a competitive choice. Therefore we manually choose a grid including `0`.
...
1. tune model complexity parameters first, such as `max_depth` and `gamma`
...
XGBoost has many hyperparameters, so an exhaustive search over all possible combinations can be computationally expensive. One of the main goals of this lecture is to introduce practical strategies for reducing the tuning time.

In this example, in addition to the tuning strategies introduced earlier, we reduce the search space by fixing some lower-priority hyperparameters at their default values. In particular, we do not tune `gamma` or `reg_alpha`. The effect of `gamma` partially overlaps with that of other tree-complexity parameters, such as `max_depth`, while `reg_alpha` and `reg_lambda` both regularize leaf weights.

For a more thorough real-world analysis, `gamma` and `reg_alpha` may also be included in the tuning process when the computational budget allows.
:::
...
### Refined grid search
Now we can run refined grid search using `n_estimators=best_round` (which is `{python} best_round` in this example). 
