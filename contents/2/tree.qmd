...
- A branch connects two nodes. For example, the branch from node #0 to node #1 corresponds to the outcome $x_0\leq 2.45$.
...
::: {#exm-}
## The split cost
The cost comparison can be understood by considering whether a subtree should be retained or pruned. Let $T_t$ be the subtree of $T$ rooted at an internal node $t$, and let $T^{(-t)}$ be the tree obtained by replacing $T_t$ with a single leaf $t$.

```{mermaid}
flowchart TB
    subgraph Before["Before pruning: T"]
        direction TB
        A[unchanged part] --> S((T_t))
    end

    subgraph After["After pruning: T^(-t)"]
        direction TB
...
The leaf $t$ is already counted in $T^{(-t)}$. Expanding it into $T_t$ therefore adds $|L(T_t)|-1$ leaves, so
$$
|L(T)|=|L(T^{(-t)})|+|L(T_t)|-1.
$$
...
$$
\begin{split}
C_{\alpha}(T^{(-t)})- C_{\alpha}(T)
&=\qty(J(T^{(-t)})+\alpha|L(T^{(-t)})|)-\qty(J(T)+\alpha|L(T)|)\\
&=\qty(J(T^{(-t)})-J(T))-\alpha(|L(T_t)|-1).
\end{split}
$$
The subtree is worth retaining only when the cost is reduced $C_{\alpha}(T)<C_{\alpha}(T^{(-t)})$, which means that
$$
J(T^{(-t)})-J(T)
>
