...
plot_tree(clf, filled=True, impurity=True, node_ids=True);  # fmt: skip
```
...
ax.set_ylim(ymin, ymax);  # fmt: skip
```
<!-- 
...
2. The random state can be fixed by setting `random_state`. The randomness happens because the model select a ramdom feature to split when multiple features are ranking the same.
...
This dataset contains 150 records with 3 classes. The dataset is prepared as follows.
...
The last `ccp_alpha` is too big that it always force the tree has only one node. In most cases this is not interesting at all. Therefore usually we would like to remove the last `ccp_alpha`. 
...
for ccp_alpha in ccp_alphas_plot:
...
ax[0].plot(ccp_alphas_plot, node_counts, marker="o", drawstyle="steps-post")
ax[1].plot(
    ccp_alphas_plot, train_acc, marker="o", drawstyle="steps-post", label="train"
)
ax[1].plot(ccp_alphas_plot, test_acc, marker="o", drawstyle="steps-post", label="test")
