...
We also tend to set some early stopping conditions to limit the growth of a tree. Some common early stopping conditions are:
...
The main tuning problem is therefore choosing the appropriate tree size. Tree size controls the bias-variance tradeoff.


Pre-pruning is simple, but it can be too greedy. A split that appears only mildly useful at an early stage may later enable highly informative splits deeper in the tree. If we stop too early, we may miss important structure in the data.

For this reason, the standard strategy for building a tree model is:

1. Grow a large tree.
2. Prune it back.
3. Use validation or cross-validation to choose the final tree size.
...
ax[1].plot(ccp_alphas_plot, train_acc, marker="o", drawstyle="steps-post", label="train")
