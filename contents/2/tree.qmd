...
For classification trees, we need a criterion to measure whether a split is good or bad. In this notes we focus on Gini impurity. 

### Gini Impurity

Assume that a node contains $n$ observations divided into $K$ classes.

Let $p_i=\frac{n_i}{n}$ be the proportion of class $i$.
...
For regression trees, CART typically minimizes the residual sum of squares (RSS). Sometimes the mean squared error (MSE) is used. These two are essentially the equaivalent. 
...
where $\bar y_l$ and $\bar y_r$ are the mean responses inside the two regions. The best split is the one that minimizes this quantity.
...
A tree makes predictions using only observations inside a small region of the feature space. Suppose a tree partitions the feature space into regions. The prediction at a point $x$ depends only on the region containing $x$. Therefore, if a new observation falls into a region, the tree ignores all data outside the region.

As a result, tree predictions are local:
...
The resulting prediction surface is piecewise constant, not a single linear equation. Therefore, trees are nonlinear models.
...
Notice that the prediction depends on the combination of all three conditions simultaneously.

In other words, the effect of $x_3$ is not independent of $x_1$ and $x_2$. The split on $x_3$ is only relevant inside the region defined by the earlier splits.
