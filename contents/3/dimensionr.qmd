...
If we use the PCA scores as predictors in a linear regression, we obtain principal component regression (PCR). PCR adds a supervised prediction step, but the PCA directions themselves are still chosen without using $y$.
:::
...
Usually we pick the dimension based on a threshold for how much variance it explains.
...
We fit PCA on the full digits dataset, and then visualize the reconstruction of one image (0-th image).
...
We can also measure the per-image storage after the PCA loading matrix has
been learned and shared across the dataset. Each original image stores 64 pixel values. With $m$ components, each image is represented by $m$ scores. Ignoring the shared loading matrix, the theoretical per-image storage ratios are:
...
These measured ratios are slightly larger than the per-image ratios above because they include the shared loading matrix and the feature means, not only the score values for each image.

For a single image alone, PCA is not necessarily a compression method, because the loading matrix must also be stored. The compression is useful when many observations share the same loading matrix and each observation only needs its own low-dimensional score vector.
...
PCA is not only used inside PCR. It can also be used as a preprocessing step before clustering, visualization, anomaly detection, nearest-neighbor methods, or other downstream analyses. This step is optional, not automatic: whether PCA helps depends on the geometry of the data and the goal of the analysis.


A common PCA preprocessing workflow for clustering is:
...
This can remove noise, reduce computation, and stabilize distances. However, PCA preserves high-variance directions, not necessarily the directions that best separate clusters. The clustering algorithm sees the derived features $Z_m=XV_m$, not the original columns of $X$.
...
The plots below show only the two signal features so that the cluster
structure is visible. However, K-means on the original features is applied
to the full matrix `X_cluster`, which also contains 200 noisy features. Those
noisy coordinates affect the Euclidean distances used by K-means, even
though they are not drawn in the figure. After PCA, the data are represented
by two score columns, so much of the noise influence is reduced before
clustering.
:::

The clustering results are displayed below. The first panel shows the true
...
This example works because PCA reduces many noisy coordinates to a smaller score representation that is more useful for this clustering task. 


In practice, PCA as preprocessing should still be chosen according to the goal of the analysis and checked against the downstream result.
