...
We learn the PCA basis from the full digits dataset, not from this single noisy image, so the fitted directions represent common digit-shaped variation.
...
The plots below show only the two signal features so that the cluster structure is visible. However, K-means on the original features is applied to the full matrix `X_cluster`, which also contains 200 noisy features. Those noisy coordinates affect the Euclidean distances used by K-means, even though they are not drawn in the figure. After PCA, the data are represented by two score columns, so much of the noise influence is reduced before
clustering.
:::
