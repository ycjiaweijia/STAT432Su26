...
In this simulated example, we set `n_clusters=3` because the data were generated from three latent groups. In real applications, the number of clusters is usually unknown and must be treated as a tuning choice.
...
For inertia, the elbow heuristic looks for the point after which additional clusters produce only modest improvement. For the silhouette score, larger values are preferred. The two diagnostics can disagree because they measure different aspects of the cluster assignments.
...
For routine analyses, it is common to keep `init="k-means++"` (which is by default) and tune `n_clusters`, while using a reasonably large `n_init`.
