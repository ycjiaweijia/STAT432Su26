...
The previous methods define clusters directly from centers, merges, or density in the observed feature space. Spectral clustering first builds a graph representation of the data and then clusters that representation:
...
The final clustering is still a partition of the original observations, but the partition is found after transforming the data into coordinates that reflect graph connectivity.

### From a Graph to New Coordinates
...
Many implementations use normalized versions of the graph Laplacian, but the
main idea is the same: eigenvectors of a Laplacian provide new coordinates that
reflect graph connectivity. A typical spectral clustering algorithm:
...
4. applies a standard clustering method, usually K-means, in that representation.
...
Spectral clustering first decides which observations are local neighbors. It
then uses the graph Laplacian to construct a low-dimensional embedding in which
observations connected through the graph tend to have similar coordinates.

After this embedding is computed, a standard clustering method is applied to the
embedded points. In `sklearn`, the common choice is K-means. Thus, spectral
clustering can separate rings or curved groups because K-means is not applied
directly to the original coordinates.
...
connections to build new coordinates. Clusters are then found in those new
coordinates, usually by K-means. Thus, hierarchical clustering groups
observations by following a merge tree, while spectral clustering groups
observations by transforming a similarity graph.
:::
