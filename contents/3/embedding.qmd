...
- All `perplexity` values separate most digit classes, indicating that the handwritten digits have a meaningful local neighborhood structure in the original high-dimensional space.
- With small `perplexity` (5–10), the embedding focuses on very local neighborhoods. Clusters are more fragmented, and some digit classes appear as multiple disconnected regions. This may indicate distinct local neighborhoods in the original feature space, but these regions should not automatically be interpreted as true subtypes.
- Increasing `perplexity` (30–50) produces more compact and stable clusters. Samples from the same digit are grouped more consistently, resulting in a cleaner overall layout.
...
- Some overlap between visually similar digits persists under all settings, reflecting ambiguity in the handwritten digits rather than an inappropriate choice of `perplexity`.
...
Overall, `perplexity` controls the trade-off between emphasizing fine local detail and producing a smoother representation of local neighborhood probabilities, rather than determining the number of clusters.

A useful `perplexity` is one that produces stable local neighborhoods across reasonable parameter choices. In practice, comparing several `perplexity` values is often more informative than selecting a single "best" visualization. When class labels are available, they should be used only after fitting to verify whether nearby points indeed correspond to similar observations.
...
However, they are not trying to preserve the same structure. PCA preserves linear variance. Spectral embedding preserves connectivity in a chosen graph. t-SNE preserves local neighborhood probabilities. UMAP preserves a weighted nearest-neighbor graph. Therefore, the right method depends on the question we want the visualization to answer.
...
The figure highlights three main comparisons: the effect of PCA preprocessing, the differences between t-SNE and UMAP, and the interpretation of the resulting embeddings.

---

**Raw versus PCA50**

- In this MNIST subset, applying PCA before t-SNE or UMAP changes the exact layouts, but the main digit neighborhoods remain similar.
- Most digit classes remain in similar local regions after PCA preprocessing, indicating that the first 50 principal components preserve the dominant information in the data.
- In this example, the PCA50 embeddings appear slightly cleaner, with smoother class boundaries and fewer scattered points. In practice, PCA can improve computational efficiency and remove low-variance noise while preserving much of the local structure of the data.

---

**t-SNE versus UMAP**

- Both methods reveal the nonlinear neighborhood structure of the handwritten digits. Images with the same label tend to occupy nearby regions, although several visually similar digit classes still overlap.
- Under the parameter settings used here, the t-SNE embedding forms a relatively continuous arrangement in which neighboring classes merge gradually into one another. This reflects its emphasis on preserving local neighbor probabilities.
- Under the parameter settings used here, the UMAP embedding is more segmented, with several elongated or curved regions connected through the neighborhood graph. Rather than producing compact circular clusters, it often preserves the connectivity of nearby regions.

---

**Interpretation**

- Local neighborhoods that appear consistently in both embeddings are more likely to reflect genuine relationships in the original data.
- The two methods produce noticeably different global layouts because they optimize different objective functions. Therefore, the relative positions, orientations, sizes, and long-range distances of the visual groups should not be interpreted quantitatively.
- Both embeddings should be regarded as visual summaries of local neighborhood structure rather than geometrically accurate maps of the original 784-dimensional feature space.

---

Overall, t-SNE and UMAP agree on much of the local structure while representing that structure in different ways. Their agreement strengthens confidence in the recovered local neighborhoods, whereas differences in the overall layouts are expected and should not be overinterpreted.
...
- Spectral Embedding constructs a nearest-neighbor graph before computing the embedding. It can reveal nonlinear organization that PCA cannot capture, although the result depends strongly on the choice of neighborhood graph.
- PCA50 + t-SNE and PCA50 + UMAP both start from the same 50-dimensional PCA representation and reveal the nonlinear neighborhood structure of the handwritten digits. Although they preserve many of the same local neighborhoods, they organize these neighborhoods differently because they optimize different objective functions.
