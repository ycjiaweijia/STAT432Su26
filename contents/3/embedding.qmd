...
There are some findings based on the plots.


- All `perplexity` values successfully separate most digit classes, indicating that the handwritten digits have a meaningful local neighborhood structure in the original high-dimensional space.
- With small `perplexity` (5–10), the embedding emphasizes very local neighborhoods. Clusters tend to be more fragmented, and several classes are split into multiple disconnected groups.
- Increasing `perplexity` (30–50) produces more compact and stable clusters. Samples from the same digit are grouped more consistently, resulting in a cleaner global layout.
- The relative arrangement of clusters changes across perplexity values. This illustrates that the global positions and distances between clusters in t-SNE should not be interpreted as reflecting the true geometry of the original high-dimensional data.
- Some overlap between visually similar digits remains under all settings. These mixed regions likely correspond to ambiguous handwriting rather than an inappropriate choice of `perplexity`.
- The overall visualization is relatively robust once `perplexity` reaches around 30. The embeddings for `perplexity` 30 and 50 are visually similar, suggesting that the discovered local structure is stable over this parameter range.

Overall, `perplexity` mainly affects the balance between preserving fine local detail and producing a smoother global organization, rather than changing the underlying cluster identities.
...
There are some findings from the plots. You are welcome to find more.

- The overall class structure is stable across all parameter settings. Most digit classes remain well separated, indicating that the underlying neighborhood relationships are largely preserved.
- Digits `0` and `6` remain isolated in every embedding, suggesting that these classes are intrinsically distinct from the others.
- Some groups of digits consistently appear close together, such as `1`, `2`, `3`, `8`, and `9`, although their relative arrangement changes across different parameter settings.
- Increasing `min_dist` mainly changes the compactness of the clusters. Small values produce dense, compact clusters, while larger values result in more diffuse clusters that occupy a larger area.
- Increasing `n_neighbors` changes the global organization of the embedding. Small values produce many isolated clusters, whereas larger values arrange nearby clusters into a more coherent overall structure.
- Although the overall geometry varies considerably with the choice of hyperparameters, the major clusters remain identifiable and their local neighborhood relationships are largely consistent.
...
For t-SNE, distances are converted into neighbor probabilities. The high-dimensional probabilities form $P=(p_{ij})$, the low-dimensional probabilities form $Q=(q_{ij})$, and t-SNE moves the plotted points so that $Q$ matches $P$. The main tuning parameter is `perplexity`, which controls the effective neighborhood size in $P$.
...
We compare them on a swiss roll dataset. This dataset is useful here because the data lie on a curved two-dimensional surface embedded in three dimensions. The color records the position along the roll, so a good nonlinear visualization should show a smooth color progression.
...
X_swiss_raw, swiss_position = make_swiss_roll(
    n_samples=1500,
    noise=0.05,
    random_state=1,
)
X_swiss = StandardScaler().fit_transform(X_swiss_raw)
...
fig.colorbar(scatter1, ax=axs, label="position along roll")
```

Both plots reveal the curved local structure that PCA would have difficulty showing directly. Nearby colors tend to remain near each other, which means both methods are preserving local neighborhoods along the roll.

The t-SNE plot is best read locally. It usually keeps nearby pieces of the roll close, but it may break the continuous surface into visually separated patches. Those gaps should not be interpreted as real gaps in the original data.

The UMAP plot is also local, but its interpretation is graph-based. It often gives a smoother sense of how neighboring parts of the roll connect, especially when `n_neighbors` is not too small. The compactness of the display is partly controlled by `min_dist`, so visual density should still be interpreted carefully.
...
| t-SNE | local neighborhood probabilities | global distances, island sizes, densities, axis directions | whether local neighborhoods or clustering labels are visually mixed |
| UMAP | local nearest-neighbor graph, with some broader structure depending on `n_neighbors` | exact distances, densities, and axis meanings | nonlinear local visualization and graph-based neighborhood structure |

We now apply all four methods to the same swiss roll data.

```{python}
#| warning: false
from sklearn.manifold import SpectralEmbedding

Z_pca2 = PCA(n_components=2, random_state=1).fit_transform(X_swiss)

Z_spectral_swiss = SpectralEmbedding(
    n_components=2,
...
embeddings = {
    "PCA": (Z_pca2, "PC1", "PC2"),
    "Spectral embedding": (Z_spectral_swiss, "SE1", "SE2"),
    "t-SNE": (Z_tsne_compare, "t-SNE1", "t-SNE2"),
    "UMAP": (Z_umap_compare, "UMAP1", "UMAP2"),
}
```
...
fig.colorbar(scatter, ax=axs, label="position along roll")
```

The PCA plot is the baseline. It projects the three-dimensional roll onto two linear directions, so different layers of the roll can overlap even though they are far apart along the surface.

The spectral embedding plot should be read as a graph-connectivity display. It uses a nearest-neighbor graph, so it can follow the curved surface better than PCA when the graph is well chosen. Its result depends strongly on the neighborhood graph.

The t-SNE and UMAP plots reveal stronger nonlinear local structure than PCA. They are better for checking whether nearby points along the surface remain nearby after embedding. The key difference is that t-SNE emphasizes local neighbor probabilities, while UMAP emphasizes a weighted nearest-neighbor graph. Therefore, their plots can agree about local neighborhoods while disagreeing about the global arrangement of the roll.
