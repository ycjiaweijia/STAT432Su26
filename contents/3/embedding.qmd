...
Spectral embedding starts by replacing the original data matrix with a graph. The rows $x_i\in\mathbb R^p$ are observations in feature space. Spectral embedding keeps the same observations, but represents their relationships by a graph: the vertices are observations, and edges connect observations that are similar, often nearest neighbors.
...
This is useful when the structure is better described by connectivity than by distances to cluster centers. Spectral clustering follows the same general workflow: it constructs a spectral representation and then assigns labels using a method such as K-means.
...
The constraints remove trivial solutions. Without them, $y=0$ minimizes the objective. Even with a nonzero vector, the constant coordinate $y_1=\cdots=y_n$ makes every coordinate difference zero, so it does not separate observations. The constraint $\norm{y}^2=1$ fixes the scale, and $y\perp \mathbf 1$ removes the constant coordinate. This formulation assumes that the graph is connected. If the graph has several connected components, the entire null space of $L$, not only the direction $\mathbf 1$, must be removed.

This optimization problem leads to eigenvectors because $L$ is symmetric and positive semidefinite. In this case all its eigenvalues are real and nonnegative. Write its orthonormal eigen-decomposition as
...
Thus $y^\top Ly$ is a weighted average of the eigenvalues. To make it small, we put weight on eigenvectors with small eigenvalues. For a graph Laplacian, $0$ is always an eigenvalue, and its multiplicity equals the number of connected components. If the graph is connected, its null space is spanned by the constant vector, which has zero graph roughness but gives every observation the same coordinate. After removing the null space, the first useful one-dimensional spectral coordinate is an eigenvector corresponding to the smallest positive eigenvalue:
...
Assume for now that the graph is connected. Consider the eigenvectors of the graph Laplacian $v_1,\ldots, v_n$, ordered by ascending eigenvalues. Each selected eigenvector gives one coordinate column for all observations. For an $m$-dimensional spectral embedding, we place the first $m$ nonconstant eigenvectors side by side:
...
3. Form a directed connectivity matrix $A$ by setting $A_{ij}=1$ if observation $j$ is a selected neighbor of observation $i$, and $A_{ij}=0$ otherwise.
4. In `sklearn` the matrix will be symmetrized as $W=\frac12(A+A^\top)$.
...
The formulas above use the unnormalized graph Laplacian $L=D-W$ for exposition. The `sklearn` implementation uses a normalized graph Laplacian and rescales its eigenvectors by the vertex degrees when constructing the output coordinates. The interpretation is the same at this level. The embedding favors coordinates that vary smoothly over the graph. However the exact matrix, constraints, and coordinates differ from the simplified derivation above.
...
- DBSCAN: With these parameters, DBSCAN keeps the two circles largely separate but splits part of the uneven-density outer circle, producing three clusters. The sparse bridge does not provide enough core points to merge the circles, although it affects the assignment of some nearby points.
...
- The converse is weaker: if two points appear close in the t-SNE plot, they were not necessarily close in the original space. Pairs with very small $p_{ij}$ receive a weaker direct pairwise penalty, although they still affect the objective indirectly through the normalization of $Q$ and the arrangement of other points.
...
The main use of t-SNE is to visualize high-dimensional data. It is not usually used as a feature representation for a downstream model. Instead, it helps us inspect whether patterns found by other methods are visible in local neighborhood structure.
...
This is a diagnostic check, not proof. The plot is useful because it makes possible problems visible quickly. Any hypothesis suggested by the visualization should be checked against the original observations, distances, or nearest-neighbor relationships.
:::
...
We use `perplexity=30` here as a common starting point for a moderate-size dataset, not a fixed default.
...
After the candidate neighbors are selected, UMAP assigns a local distance offset $\rho_i$ and a local scale $\sigma_i$ for each point. Under the usual default setting, $\rho_i$ is based on the distance from $x_i$ to its nearest nonzero-distance neighbor. This makes the nearest such neighbor receive weight $1$. The scale $\sigma_i$ controls how quickly the remaining selected neighbors lose weight as distance increases. It is calibrated so that the total local connection strength reaches a target determined by `n_neighbors`. Both $\rho_i$ and $\sigma_i$ are chosen automatically for each point rather than set directly by the user.
...
)\quad\text{ for $j$ in the selected neighbor set, otherwise $a_{ij}=0$.}.
$$
...
UMAP then moves the coordinates so that $R(Z)$ resembles $W$. Up to terms that depend only on $W$ and therefore do not affect the optimization over $Z$, this gives a cross-entropy loss of the form
...
$$
\qty(\texttt{min\_dist},\texttt{spread})
\longrightarrow
(\alpha,\beta)
\longrightarrow
r_{ij}
...
- `spread` controls the overall distance scale of the low-dimensional similarity curve. It is usually left at its default value of `1.0`, so we focus mainly on `min_dist`.
:::
...
As with t-SNE (see the note above on t-SNE as a diagnostic plot), a UMAP plot is a hypothesis to check, not proof of structure.
:::
...
Here, `n_neighbors=15` gives a moderate local neighborhood size and `min_dist=0.1` allows fairly compact visual groups — tuning choices, not universal defaults.
...
- The overall class structure is similar across the parameter settings displayed here. Most digit classes remain recognizable, suggesting that much of the local neighborhood structure persists across these settings.
...
- As with t-SNE, small `n_neighbors` can fragment a class into several local neighborhoods — not necessarily true subtypes.
...
- Although the global layout changes noticeably, the major local neighborhoods remain similar across the displayed settings. A stronger stability claim would also require varying random seeds, metrics, and preprocessing.
...
For static interpretation, the practical difference between the two methods is often small because both primarily visualize local neighborhood information. At comparable neighborhood scales:

- A stable bridge can suggest a chain of local neighbor relationships.
- A stable island can suggest a group with relatively strong internal relationships and relatively weak external relationships.

Here, "stable" means that the structure persists across reasonable choices of neighborhood parameters, random initializations, metrics, and preprocessing.

When t-SNE and UMAP disagree, the disagreement should be treated as evidence that the apparent structure is sensitive to the neighborhood representation, neighborhood scale, or optimization—not as evidence that one method is necessarily correct. If they disagree about a separation, that separation has not yet been shown to be a method-independent feature of the data.
...
Another practical difference between t-SNE and UMAP concerns new observations. Common UMAP implementations provide an approximate out-of-sample procedure that embeds a new observation using its relationships to the fitted data. The new observation is connected to nearby training observations in the fitted nearest-neighbor graph, and the resulting edge strengths are used to initialize and optimize its low-dimensional coordinates relative to the existing embedding. The original training coordinates are generally kept fixed during this procedure.

Classical t-SNE, by contrast, produces a joint layout of the observations included during fitting. Because its low-dimensional probabilities are globally normalized over pairs of observations, adding a new observation changes the joint objective. Classical t-SNE therefore does not naturally define a simple fixed out-of-sample transformation. Extensions and approximate procedures can provide such mappings, but they are not part of the classical algorithm.
:::
...
- With these settings, the UMAP embeddings show several groups of similar digits as relatively well-separated neighboring regions. For example, groups of visually similar digits, such as 3, 5, and 8 or 4, 7, and 9, tend to occupy neighboring regions, reflecting similarities in their handwritten shapes. This separation is a diagnostic pattern to investigate, not a final definition of the underlying clusters.
- With these settings, the t-SNE embeddings display more gradual visual transitions between some similar digit classes. Rather than forming clearly separated islands, observations from neighboring classes overlap through possible intermediate handwriting styles, giving the layout a more continuous appearance. Likewise, this pattern suggests a possible chain of local similarities to investigate rather than a settled conclusion — it should be verified by inspecting the original images and their high-dimensional nearest-neighbor relationships.
...
PCA, spectral embedding, t-SNE, and UMAP all create low-dimensional coordinates, but they answer different diagnostic questions. The table below summarizes what each method mainly preserves.
...
The four methods should therefore be interpreted differently because each is designed to preserve a different aspect of the original data, as summarized in the table above. None of them provides a geometrically faithful map of the original feature space; their interpretation should always be based on the structure each method is designed to preserve.
...
- In these figures, the larger sample fills in more intermediate observations, and the spectral embedding appears smoother. These are empirical patterns in this example, not general consequences of increasing the sample size.
