...
- Non-binding constraint: If the OLS solution already satisfies $\norm{\hat{\beta}_{\text{ols}}}_2^2 \le s$, then the OLS solution remains optimal for the constrained problem. Therefore, $\hat{\beta}_{\text{ridge}}=\hat{\beta}_{\text{ols}}$. The constraint has no effect, and the corresponding penalty parameter is $\lambda = 0$.
- Binding constraint: If $\norm{\hat{\beta}_{\text{ols}}}_2^2 > s$, then the OLS solution is not feasible. The constrained optimum must therefore lie on the boundary $\norm{\hat{\beta}_{\text{ridge}}}_2^2 = s$, and the corresponding penalty parameter satisfies $\lambda > 0$.

Therefore, when $\lambda>0$, the parameters $\lambda$ and $s$ are related through $\norm{(X^\top X+\lambda I)^{-1}X^\top y}_2^2=s$. 

The parameter $\lambda$ can be viewed as the "price" paid for enforcing the constraint. As $s$ decreases, the corresponding value of $\lambda$ increases, producing greater shrinkage toward the origin. The norm of the ridge estimator decreases as $\lambda$ increases. 

Consequently, for every $s < \norm{\hat{\beta}_{\text{ols}}}_2^2$, there exists a unique $\lambda>0$ such that $\norm{\hat{\beta}_{\text{ridge}}}_2^2=s$.

This relationship is consistent with the KKT complementary slackness condition in optimization theory,
